	...
	public function save( $post, $postID ) {
		...
		if ( ! empty( $formSettings->form_captcha ) ) {
			if ( $formSettings->form_captcha == 1 ) {
				include_once ( IG_UNIFORM_PATH . 'libraries/3rd-party/recaptchalib.php' );
				...
				$resp = recaptcha_check_answer( IG_UNIFORM_CAPTCHA_PRIVATEKEY, $_SERVER[ 'REMOTE_ADDR' ], $recaptchaChallenge, $recaptchaResponse );

				if ( ! $resp->is_valid ) {
					$return->error[ 'captcha' ] = __( 'IG_UNIFORM_ERROR_CAPTCHA', IG_UNIFORM_TEXTDOMAIN );
				...
				if ( ! empty( $_POST[ 'form_name' ] ) && ! empty( $_POST[ 'captcha' ] ) ) {
					...
					if ( strtolower( $sCaptcha ) != strtolower( $_POST[ 'captcha' ] ) ) {
						$return->error[ 'captcha_2' ] = __( 'IG_UNIFORM_ERROR_CAPTCHA', IG_UNIFORM_TEXTDOMAIN );
						return $return;
					}
				}
				else {
